...
describe('Exchange', () => {
  ...
  describe('getTokenAmount', async () => {
    it('correct get Token output amount', async () => {
      ...
      expect(
        toEther(
          await exchange.getOutputAmount(toWei(1), etherReserve, tokenReserve)
        )
  ...
  describe('getEthAmount', async () => {
    it('correct get Eth output amount', async () => {
      ...
      expect(
        toEther(
          await exchange.getOutputAmount(toWei(1), tokenReserve, etherReserve)
        )
  ...
  describe('EthToTokenSwap', async () => {
    it('correct EthToTokenSwap', async () => {
      ...
      expect(toEther(await token.balanceOf(user.address))).to.eq(
        '3.996003996003996003'
      );
    });
  });
